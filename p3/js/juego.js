...
function usar(){
    ...
    let ficha = document.getElementById(numero),
        partidaJSON = sessionStorage.getItem("partida");
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (canvas) {
        ...
        const rect = canvas.getBoundingClientRect(),
            ...
            columna = Math.floor(x / cellSize),
            fila = Math.floor(y / cellSize),
            seleccionada = sessionStorage.getItem("seleccionada");
    ...
    canvas.addEventListener('mousemove', function(event) {
        const rect = canvas.getBoundingClientRect(),
            ...
            { fila, columna } = getCasillaPos(mouseX, mouseY);
...
//comprobar si hay múltiplo de 5
function comprobar(){
    let partidaJSON=sessionStorage.getItem("partida");
    const partida = JSON.parse(partidaJSON);
    var numeros= partida.numeros,
